...
namespace FlightControlDB.Repositories
{
    public class Repository : IRepository
    {
        ...
        //    return res;
        //}
        public async Task<Logger> GetLogAsync(DateTime dateIn, int flightId) 
        {
            return await _data.Loggers.FirstOrDefaultAsync(l => l.Flight.Id == flightId && l.In == dateIn);
        }
        public async Task<List<Leg>> NextLegsAsync(Leg leg)
        {
            //return await _data.Legs.FirstOrDefaultAsync(l => leg.NextLegs.HasFlag(l.CurrentLeg));


            return await _data.Legs.Where(l => leg.NextLegs.HasFlag(l.CurrentLeg)).ToListAsync();
